...
public class IoRequest extends Serializer {
    private short agvid;
    ...
    public void setIo_action(int io_action) {
        this.io_action = io_action;
    }
    ...
    protected void write() {
        ...
        for(int i = 2;i>0; i--){
            arr[i] = io_action % 10;
            io_action = (io_action - arr[i])/10 ;
        }

        try{
            for(int i = 0; i < 3; i++)
                if(arr[i]>= 0 && arr[i] <= 2 )
                    writeByte((byte) arr[i]);
                else
